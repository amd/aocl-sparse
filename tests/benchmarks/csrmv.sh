...
while IFS= read -r filename; do
    echo "numactl --physcpubind=4 $bench --function=csrmv --precision=d --alpha=1 --beta=0 --iters=1 --mtx=$filename --verify=1"
    numactl --physcpubind=4 $bench --function=csrmv --precision=d --alpha=1 --beta=0 --iters=1 --mtx=$filename --verify=1 2>&1 | tee -a $logname
