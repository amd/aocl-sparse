...
for filename in ./matrices/*.mtx; do
    $bench -f csrmv --precision s --alpha 1 --beta 0 --iters 1000 --mtx $filename 2>&1 | tee -a $logname
done
# Run csrmv for all matrices available
for filename in ./matrices/*.csr; do
    $bench -f csrmv --precision s --alpha 1 --beta 0 --iters 1000 --csr $filename 2>&1 | tee -a $logname
done
