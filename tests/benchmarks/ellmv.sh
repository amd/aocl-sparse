...
logname=dellmv_$(date +'%Y%m%d%H%M%S').csv
truncate -s 0 $logname

# Run ellmv for all matrices in ./matrices/matrixlist
while IFS= read -r filename; do
    echo "numactl --physcpubind=4 $bench -f ellmv --precision d --alpha 1 --beta 0 --iters 1000 --mtx $filename --verify 1"
    numactl --physcpubind=4 $bench -f ellmv --precision d --alpha 1 --beta 0 --iters 1000 --mtx $filename --verify 1 2>&1 | tee -a $logname
